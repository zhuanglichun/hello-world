...
# df5 = df1 [df1>0]
# df6 = df5.dropna()
df = pd.read_csv('E:/727dropnandata/afterdata.csv')
df1 = df[['user_truename', 'gamename', 'mean_score']]  # 只选取有实际作用的列
grouped = df1['mean_score'].groupby([df1['user_truename'], df1['gamename']])
df2 = grouped.median()
df3 = df2.reset_index()
...
old_gameset = np.unique(df3['gamename'])
old_gamelist = list(old_gameset)
fid = np.arange(len(old_gamelist))
df3['fid'] = df3['gamename'].replace(old_gamelist,fid)
df3['fid'] = df3['fid']+1
...
tables1 = pd.pivot_table(df3, index='fid', values='mean_score', aggfunc=lambda x: (np.max(x) - np.min(x)) * 0.90)["mean_score"]
tables2 = pd.pivot_table(df3, index='fid', values='mean_score', aggfunc=lambda x: (np.max(x) - np.min(x)) * 0.70)["mean_score"]
tables3 = pd.pivot_table(df3, index='fid', values='mean_score', aggfunc=lambda x: (np.max(x) - np.min(x)) * 0.50)["mean_score"]
tables4 = pd.pivot_table(df3, index='fid', values='mean_score', aggfunc=lambda x: (np.max(x) - np.min(x)) * 0.30)["mean_score"]
tables5 = pd.pivot_table(df3, index='fid', values='mean_score', aggfunc=lambda x: (np.max(x) - np.min(x)) * 0.10)["mean_score"]
...
print("ok")
score_table.loc[(score_table["mean_score"] < score_table['e']), 'score'] = 6 #
score_table.loc[(score_table["mean_score"] >= score_table['e']) & (score_table["mean_score"] < score_table['d']), 'score'] = 5
score_table.loc[(score_table["mean_score"] >= score_table['d']) & (score_table["mean_score"] < score_table['c']), 'score'] = 4
score_table.loc[(score_table["mean_score"] >= score_table['c']) & (score_table["mean_score"] < score_table['b']), 'score'] = 3
score_table.loc[(score_table["mean_score"] >= score_table['b']) & (score_table["mean_score"] < score_table['a']), 'score'] = 2
score_table.loc[(score_table["mean_score"] >= score_table['a']), 'score']=1
 # 将所有的得分离散化在等级里面
...
score_table.to_csv('E:/727dropnandata/score_tabledengji.csv',index = None)
...
scores=score_table['score'].values
y=score_table['score'].values
# from keras.utils import np_utils
# from sklearn.preprocessing import LabelEncoder
# # encode class values as integers
# encoder = LabelEncoder()
# encoded_Y = encoder.fit_transform(scores)
...
model.save_weights('model727dropnandata.h5')
json_string = model.to_json()
with open("model727dropnandata.json", "w") as json_file:
...
pd_pred_test = pd.DataFrame(pred_test)
pd_pred_test.to_csv('E:/727dropnandata/preddata_test.csv',index = None)

pd_y_test = pd.DataFrame(y_test)
pd_y_test.to_csv('E:/727dropnandata/pd_y_test.csv',index = None)


pred = model.predict(x)
pd_pred = pd.DataFrame(pred)
pd_pred.to_csv('E:/727dropnandata/preddata.csv',index = None)
print('ok')
