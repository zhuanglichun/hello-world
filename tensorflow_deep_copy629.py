...
print("ok")
score_table.loc[(score_table["score"] < score_table['e']), 'score'] = 5 #
score_table.loc[(score_table["score"] >= score_table['e']) & (score_table["score"] < score_table['d']), 'score'] = 4
score_table.loc[(score_table["score"] >= score_table['d']) & (score_table["score"] < score_table['c']), 'score'] = 3
score_table.loc[(score_table["score"] >= score_table['c']) & (score_table["score"] < score_table['b']), 'score'] = 2
score_table.loc[(score_table["score"] >= score_table['b']) , 'score'] = 1
 # 将所有的得分离散化在等级里面
...
pred_allmax = np.argmax(all1,1)+1
print('ok')


print('ok')
pd_allmax = pd.DataFrame(pred_allmax)
pd_allmax.to_csv('E:/lf/pred_all.csv',index = None)



print('ok')
c = np.abs(np.array(score_table)-np.array(pd_allmax))
d = c.sum()
